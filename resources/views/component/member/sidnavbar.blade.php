...
<html lang="en">
  <head>
    ...
    <title>YPCSTORE - INDEX</title>

    @include('component.member.css')
    <script
  ...
  <body>
    <aside class="sidebar offcanvas-lg offcanvas-start bg-white shadow">
      ...
      <div class="logo-brand mt-lg-5">
        <img
          src="{{ asset('dashboard/images/logoypc.png')  }}"
          alt="Logo Shoso"
          width="52"
          height="50"
        />
        <div>
          <h6 class="title-store">YPC Store</h6>
          <p class="tagline-store">easier shopping</p>
        </div>
      </div>
      <hr />
        <nav class="menu flex-fill">
          <div class="section-menu">
            <a href="/" class="item-menu {{ (Request::is('/'))?'active':'' }}" onclick="handleClickMenu(this)">
          ...
          <div class="section-menu">
            ...
            <a href="{{ url('/favorite') }}" class="item-menu" onclick="handleClickMenu(this)">
        ...
        <section>
          ...
          <nav class="nav-content gap-5">
            ...
            <div class="d-flex gap-3 align-items-center">
              ...
              @if (getphoto() == null)
              <img
              src="{{ asset('assets/images/guest.jpeg') }}"
              alt="Photo Profile"
              class="photo-profile"
              />
              @else
              <img src="{{ asset('storage/'.getphoto()) }}" alt="{{ getphoto() }}" class="photo-profile"
              />
              @endif    
              @endauth
              <div>
                ...
                <p class="subtitle-content m-2">
                  {{ Verifikasi() }}
                </p>
              </div>
                @else
                <p class="title-content mb-2">Welcome in YPC Store</p>
              </div>
                @endauth
              </div>
              </div>
              <div class="search-wrapper d-flex align-items-center" style="gap: 25px;">
                @if (Request::is('yourorder'))
                <form action="{{ url('yourorder') }}" method="get" class="search-bar flex-fill d-flex bg-white rounded-5">
                  <input
                    type="date"
                    placeholder="Search"
                    name="tcari"
                ...
                @elseif (Request::is('explore'))
                    <form action="{{ url('explore') }}" method="get" class="search-bar flex-fill d-flex bg-white rounded-5">
                      <input
                        type="text"
                        placeholder="Search"
                        name="tcari"
                ...
                @elseif (Request::is('favorite'))
                    <form action="{{ url('search') }}" method="get" class="search-bar flex-fill d-flex bg-white rounded-5">
                      <input
                        type="text"
                        placeholder="Search"
                        name="tcari"
                ...
                @elseif (Request::is('/'))
                    <form action="{{ url('/') }}" method="get" class="search-bar flex-fill d-flex bg-white rounded-5">
                      <input
                        type="text"
                        placeholder="Search"
                        name="tcari"
                ...
                <div>
                  <a href="">
                    <svg xmlns="http://www.w3.org/2000/svg" color="black" width="16" height="16" fill="currentColor" class="bi bi-bell" viewBox="0 0 16 16">
                ...
                <div>
                  <a href="{{ url('cart') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" color="black" width="16" height="16" fill="currentColor" class="bi bi-cart4" viewBox="0 0 16 16">
                ...
                @else
                <div>
                  <a href="/login" class="nav-link" onclick="handleClickMenu(this)">
                    <p>Login</p>
                  </a>
                  </div>
                @endauth
